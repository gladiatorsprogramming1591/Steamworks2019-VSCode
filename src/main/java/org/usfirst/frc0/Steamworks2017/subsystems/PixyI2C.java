...
//		values = new PixyPacket();
	...
	*/
	...
	public PixyPacket readPacket(int Signature) throws PixyException { //The signature should be which number object in 
		...
		for (int i = 0; i <= 1; i++) { // normally should be 16, trying only 1 cycle for now
			...
			if (syncWord == 0xaa55) { //Check is first 2 bytes equal a "sync word", which indicates the start of a packet of valid data
				...
				else { // double sync word
					SmartDashboard.putNumber("doublesyncword: ", syncWord);
				}
				if(true) break; // for now when only 4 bytes, break after processing 4 bytes
				...
				SmartDashboard.putNumber("nonsyncword: ", syncWord);
		}
